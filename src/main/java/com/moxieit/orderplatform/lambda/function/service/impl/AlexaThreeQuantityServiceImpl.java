...
public class AlexaThreeQuantityServiceImpl implements AlexaService {
	...
	public BaseResponse serveLex(AlexaDTO alexaDTO, Context context) {
		...
		if (firstPage.iterator().hasNext()) {
			...
			Number itemCost = 3 * Double.parseDouble(itemPrice);
			UpdateItemSpec updateItemSpec = new UpdateItemSpec().withPrimaryKey("uuid", order.getString("uuid"))
					...
					.withValueMap(new ValueMap().withNumber(":val", (Number) itemCost).withNumber(":qua", (Number)3));
